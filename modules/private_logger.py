...
def ensure_daily_log_html(output_format=None):
    ...
    begin_part = (
        f"<!DOCTYPE html><html><head><title>Fooocus Log {date_string}</title>{css_styles}</head>"
        f"<body><p>Fooocus Log {date_string} (private)</p>"
        f"<!--fooocus-log-split-->\n\n"
    )
    end_part = "\n<!--fooocus-log-split--></body></html>"
    with open(html_name, 'w', encoding='utf-8') as f:
        f.write(begin_part + end_part)

    return html_name


def log(img, metadata, metadata_parser: MetadataParser | None = None, output_format=None, task=None, persist_image=True, filename_prefix=None) -> str:
    ...
    date_string, local_temp_filename, only_name = generate_temp_filename(
        folder=path_outputs, extension=output_format, prefix=filename_prefix)
